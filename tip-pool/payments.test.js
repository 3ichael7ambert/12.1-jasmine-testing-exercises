//FUNCTIONS
// submitPaymentTable(curPayment)
//createCurPayment()
// appendPaymentTable(curPayment) 

//  updateServerTable();
//updateSummary();



// describe('Submit Payment Info', function() {

//     beforeEach( function() {
//         curPayment = true;
//     });

//     it('', function() {
//       expect(curPayment(true)).toBe(true);
//     });
//   });


// describe('Current Payment', function() {

//     beforeEach( function() {
//         billAmtInput.value = 100;
//         tipAmtInput.value = 10;
//         // let billAmt = billAmtInput.value;
//         // let tipAmt = tipAmtInput.value;
//     });

//     it('', function() {
//       expect( ).toEqual();
//     });
//   });


describe("Payments test (with setup and tear-down)", function() {
    beforeEach(function () {
      billAmtInput.value = 1000;
      tipAmtInput.value = 150;
    });
  
    it('should add a new payment to allPayments on submitPaymentInfo()', function () {
      submitPaymentInfo();
  
      expect(Object.keys(allPayments).length).toEqual(1);
      expect(allPayments['payment1'].billAmt).toEqual('1000');
      expect(allPayments['payment1'].tipAmt).toEqual('150');
      expect(allPayments['payment1'].tipPercent).toEqual(15);
    });
  
    it('should not add a new payment on submitPaymentInfo() with empty input', function () {
      billAmtInput.value = '';
      submitPaymentInfo();
  
      expect(Object.keys(allPayments).length).toEqual(0);
    });
  

    ////////////////////////////////
    it('should payment update #paymentTable on appendPaymentTable()', function () {
      let curPayment = createCurPayment();
      allPayments['payment1'] = curPayment;
  
      appendPaymentTable(curPayment);
  
      let curTdList = document.querySelectorAll('#paymentTable tbody tr td');
  
      expect(curTdList.length).toEqual(3);
      expect(curTdList[0].innerText).toEqual('$1000');
      expect(curTdList[1].innerText).toEqual('$150');
      expect(curTdList[2].innerText).toEqual('15%');
    //   expect(curTdList[3].innerText).toEqual('X');
    });
    //////////////////////////////

  
    it('test creating new paymenton createCurPayment()', function () {
      let expectedPayment = {
        billAmt: '1000',
        tipAmt: '150',
        tipPercent: 15,
      }
  
      expect(createCurPayment()).toEqual(expectedPayment);
    });
  
    it('check empty payment values, should do nothing/undefined createCurPayment()', function () {
      billAmtInput.value = '';
      tipAmtInput.value = '';
      let curPayment = createCurPayment();
  
      expect(curPayment).toEqual(undefined);
    });
  
    afterEach(function() {
      billAmtInput.value = '';
      tipAmtInput.value = '';
      paymentTbody.innerHTML = '';
      summaryTds[0].innerHTML = '';
      summaryTds[1].innerHTML = '';
      summaryTds[2].innerHTML = '';
      serverTbody.innerHTML = '';
      paymentId = 0;
      allPayments = {};
    });
  });





